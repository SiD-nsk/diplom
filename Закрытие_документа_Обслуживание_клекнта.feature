...
Контекст:
	Дано я подключаю TestClient "Фирма IT" логин "Специалист" пароль ""
	И я закрываю все окна клиентского приложения

Сценарий: Закрытие Документа Обслуживание клиента Специалмст Лодочкин, 03.05.2026 с 08 по 12
...
Если таблица "Список" содержит строки: Тогда
		...
		| '08.00'                     | '12.00'                        | '03.05.2026'            | 'Абонентское обслуживание №10' | 'Глубокое озеро' |  'Скороходов К.К.' | 'Ожидается выполнение работ' |
...
Если таблица "Список" содержит строки: Тогда
		| 'Время начала работ (План)' | 'Время окончания работ (План)' | 'Дата проведения работ' | 'Договор'                      | 'Клиент'         |  'Специалист'     | 'Статус заявки'     |
		| '08.00'                     | '12.00'                        | '03.05.2026'            | 'Абонентское обслуживание №10' | 'Глубокое озеро' | | 'Скороходов К.К.' | 'Заявка выполнена' |
...
Сценарий: Закрытие Документа Обслуживание клиента Специалмст Ломов М.М., 04.04.2026 с 10 по 12

И В командном интерфейсе я выбираю 'Обслуживание клиентов' 'Обслуживание клиентов'
Тогда открылось окно 'Обслуживание клиентов'
Если таблица  "Список" содержит строки: Тогда
	| 'Время начала работ (План)' | 'Время окончания работ (План)' |  'Дата проведения работ' | 'Договор'                      | 'Клиент'         		|  'Специалист'     | 'Статус заявки'              |
	| '10.00'                     | '12.00'                        |  '04.04.2026'            | 'Абонентское обслуживание №12' | 'Марианская впадина' 	|  'Ломов М.М.' 	| 'Ожидается выполнение работ' |
...
Если таблица "Список" содержит строки: Тогда
		| 'Время начала работ (План)' | 'Время окончания работ (План)' |  'Дата проведения работ' | 'Договор'                      | 'Клиент'         		|  'Специалист'     | 'Статус заявки'              |
		| '10.00'                     | '12.00'                        |  '04.04.2026'            | 'Абонентское обслуживание №12' | 'Марианская впадина' 	|  'Ломов М.М.' 	| 'Заявка выполнена' |
...
Сценарий: Закрытие Документа Обслуживание клиента Специалист Самокатов С. С., 05.05.2026 с 10 по 15

И В командном интерфейсе я выбираю 'Обслуживание клиентов' 'Обслуживание клиентов'
Тогда открылось окно 'Обслуживание клиентов'
Если таблица  "Список" содержит строки: Тогда
	| 'Время начала работ (План)' | 'Время окончания работ (План)' |  'Дата проведения работ' | 'Договор'                      | 'Клиент'         		|  'Специалист'     | 'Статус заявки'              |
	| '10.00'                     | '15.00'                        |  '05.05.2026'            | 'Абонентское обслуживание №15' | 'ООО Райский остров' 	|  'Самокатов С.С.' 	| 'Ожидается выполнение работ' |
...
Если таблица "Список" содержит строки: Тогда
		| 'Время начала работ (План)' | 'Время окончания работ (План)' |  'Дата проведения работ' | 'Договор'                      | 'Клиент'         		|  'Специалист'     | 'Статус заявки'              |
		| '10.00'                     | '15.00'                        |  '05.05.2026'            | 'Абонентское обслуживание №15' | 'ООО Райский остров' 	|  'Самокатов С.С.' 	| 'Заявка выполнена' |
